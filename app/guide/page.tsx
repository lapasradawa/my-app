...
const sections = [
  {
    ...
    subtitle: 'หน้าหลักสำหรับบันทึกและติดตาม Invoice สินค้าขาเข้า',
    steps: [
      { label: 'บันทึก Invoice ใหม่', desc: 'กรอกข้อมูล Invoice No., สกุลเงิน (CNY/USD), จำนวนเงิน, BL Date, วันที่สินค้าเข้าคลัง และ Exchange Rate แล้วกด "เพิ่ม Invoice"' },
      { label: 'ติดตามสถานะ', desc: 'ระบบแสดงรายการ Invoice ทั้งหมด พร้อมสถานะ เช่น "อยู่ที่จีน", "On board", "เข้าคลัง" — กดที่ Invoice No. เพื่อดูรายละเอียด' },
      { label: 'บันทึก Cost Saving', desc: 'ในหน้า Detail กรอก Cost Saving (THB) และ % ที่ประหยัดได้เทียบกับ Estimate' },
      { label: 'Exchange Rate หลายรายการ', desc: 'ถ้า Invoice มีการแลกเปลี่ยนเงินหลายครั้ง (ต่างอัตรา) สามารถเพิ่มได้หลายรายการ ระบบจะรวมยอดให้อัตโนมัติ' },
    ],
    tips: ['ข้อมูลที่กรอกจะแสดงใน Report และ Dashboard โดยอัตโนมัติ', 'BL Date จะถูกนำไปคำนวณ Due Date (BL Date + 30 วัน) ให้อัตโนมัติ'],
  },
  {
    href: '/dashboard',
    label: 'Dashboard',
    ...
    subtitle: 'ดูสถานะสินค้าทุก Invoice แบบ Real-time ในหน้าเดียว',
    steps: [
      { label: 'ดูภาพรวม', desc: 'แสดงรายการ Invoice ทั้งหมดพร้อมสถานะ, ยอด FOB, Cost Saving และ Due Date โดยจัดกลุ่มตามเดือนที่สินค้าเข้าคลัง' },
      { label: 'กรองสถานะ', desc: 'กดปุ่มด้านบนเพื่อกรองเฉพาะ Invoice ที่ต้องการดู เช่น เฉพาะสินค้าที่ยังอยู่ที่จีน หรือเฉพาะที่ชำระแล้ว' },
      { label: 'ดูรายละเอียด', desc: 'กดที่ Invoice No. (ข้อความสีน้ำเงิน) เพื่อเปิดหน้า Detail และแก้ไขข้อมูลได้' },
    ],
    tips: ['Dashboard อัปเดตทันทีเมื่อมีการบันทึกข้อมูลใหม่', 'ต้องปลดล็อก (ใส่รหัส) ก่อนแก้ไขข้อมูลใดๆ'],
  },
  {
    href: '/report',
    label: 'Report',
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-4">
        ...
        <Link href="/guide" className="text-sm text-blue-600 font-semibold">คู่มือ</Link>
