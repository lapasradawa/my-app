        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-5">
          ...
          <div className="border border-gray-100 rounded-lg p-3 mb-4 bg-gray-50">
            <div className="flex items-center justify-between mb-2">
              ...
              {!editingNames && (
                <button onClick={() => requireUnlock(() => setEditingNames(true))} className="text-xs text-blue-500 hover:text-blue-700">
              ...
              <div className="space-y-2">
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input type="text" value={newChNo} onChange={e => setNewChNo(e.target.value)}
                      placeholder="เช่น RBSYG01-GEN5"
                      className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm outline-none focus:border-blue-400" autoFocus />
                  </div>
                  <div>
                    <label className="text-xs text-gray-500 mb-1 block">PO RBS TH No.</label>
                    <input type="text" value={newThNo} onChange={e => setNewThNo(e.target.value)}
                      placeholder="เช่น POSOR2607-0002"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 pt-4 border-t border-gray-100">
              ...
              {editingDate ? (
                <div className="flex items-center gap-2 flex-wrap">
                  <input type="date" value={newDate} onChange={e => setNewDate(e.target.value)}
                    className="border border-blue-300 rounded px-2 py-1 text-sm outline-none focus:border-blue-500" autoFocus />
                  <button onClick={saveDate} disabled={savingDate} className="text-xs text-blue-600 hover:text-blue-800 font-medium">{savingDate ? '...' : 'บันทึก'}</button>
                ...
                <div className="flex items-center gap-2">
                  <p className="text-sm font-medium text-gray-800">
                    {po.po_date ? new Date(po.po_date).toLocaleDateString('th-TH', { day: '2-digit', month: 'short', year: 'numeric' }) : <span className="text-gray-400">—</span>}
                  </p>
            ...
            {/* Cost Saving inline */}
            <div className="lg:col-span-2 border-l border-gray-100 pl-4">
              <div className="flex items-center justify-between mb-1">
                <p className="text-xs text-gray-400">Cost Saving</p>
                {!editCost && (
                  <button onClick={() => requireUnlock(() => { setCostInput(po.cost_saving != null ? String(po.cost_saving) : ''); setCostPctInput(po.cost_saving_pct != null ? String(po.cost_saving_pct) : ''); setEditCost(true) })}
                    className="text-xs text-blue-500 hover:text-blue-700">
                    {po.cost_saving != null ? 'แก้ไข' : '+ เพิ่ม'}
                  </button>
                )}
              </div>
              {editCost ? (
                <div className="flex flex-col gap-1.5">
                  <input type="number" value={costInput} onChange={e => setCostInput(e.target.value)} placeholder="Cost Saving (THB)"
                    className="border border-gray-300 rounded-lg px-2 py-1 text-sm outline-none focus:border-blue-400 w-full" autoFocus />
                  <input type="number" value={costPctInput} onChange={e => setCostPctInput(e.target.value)} placeholder="% Cost Saving"
                    className="border border-gray-300 rounded-lg px-2 py-1 text-sm outline-none focus:border-blue-400 w-full" />
                  <div className="flex gap-2 mt-0.5">
                    <button onClick={() => setEditCost(false)} className="text-xs px-2 py-1 text-gray-500 hover:bg-gray-100 rounded-lg">ยกเลิก</button>
                    <button onClick={saveCostSaving} disabled={savingCost} className="text-xs px-2 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-800">{po.cost_saving != null ? `${po.cost_saving.toLocaleString()} THB` : <span className="text-gray-400 font-normal">—</span>}</p>
                  <p className="text-sm font-semibold text-green-600">{po.cost_saving_pct != null ? `${po.cost_saving_pct}%` : <span className="text-gray-400 font-normal text-xs">ยังไม่มีข้อมูล</span>}</p>
                  <div className="mt-2">
                    {po.cost_saving_file_url ? (
                      <div className="flex items-center gap-2">
                        <a href={po.cost_saving_file_url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:text-blue-800 font-medium">📊 ดาวน์โหลดไฟล์</a>
                        <button onClick={() => requireUnlock(() => costFileRef.current?.click())} className="text-xs text-gray-400 hover:text-gray-600">{uploadingCostFile ? '...' : 'เปลี่ยน'}</button>
                      </div>
                    ) : (
                      <button onClick={() => requireUnlock(() => costFileRef.current?.click())} disabled={uploadingCostFile}
                        className="text-xs border border-dashed border-gray-200 rounded px-2 py-1 text-gray-400 hover:border-blue-300 hover:text-blue-500 transition-colors disabled:opacity-50">
                        {uploadingCostFile ? 'กำลังอัปโหลด...' : '+ อัปโหลดไฟล์ Cost Saving'}
                      </button>
                    )}
                  </div>
                </div>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-gray-50 text-gray-500 text-xs border-b border-gray-200">
                ...
                <th className="px-4 py-3 text-left min-w-[180px]">Item Code</th>
                <th className="px-4 py-3 text-left">Description</th>
                <th className="px-4 py-3 text-right w-24">QTY</th>
                <th className="px-4 py-3 text-right w-40">Unit Price ({po.currency})</th>
                <th className="px-4 py-3 text-right w-40">Total ({po.currency})</th>
              </tr>
            </thead>
            <tbody>
              {po.rows.map((row, i) => (
